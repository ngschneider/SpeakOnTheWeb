#### LOGIN REQUIRMENTS ####
### LOGIN IS SITE SPECIFIC, SO THE FORM ELEMENTS NEED TO BE KNOW FOR EACH SITE.
...
# Key is the site to LOGIN, Value a truple of the element type and element name
# Should be in the form (TYPE,username,Type, password TYPE, subbit)
elementTag = {
...
#       String : Name of site 
def isLogin(site: str):
    if(site in loginable):
        return (True,loginable.get(site))
    else:
        return (False,"")
...
def requirment(site: str, command):
    ...
    searchRequirment = isLogin(siteName)
